...
//			flag = false,
		...
		Zepto(function() {
			...
			poster.initTouch($pic[0], $picBg[0]);
				...
				if (files && files.length > 0) {
					...
					reader.onload = function() {
						...
						img.onload = function() {
							...
							$picBg.data('width', this.width); //实际宽度
							$picBg.data('height', this.height); //实际高度
	
							var realImg = new Image();
							realImg.onload = function() {
								$picBg.attr('src', realImg.src); //第三次载入Base64数据
								loadingHide();
								isDoning = false;
								$camera.hide();
							};
			...
			$('#btnRotate').on('mousestart', function() {
				if ($picBg.attr('src') == 'img/pic-bg.jpg') {
					return;
				}
				direction++;
				if (direction > 3) {
				...
				if (rotates[direction]) {
					$picBg.data('width', rotates[direction].width);
					$picBg.data('height', rotates[direction].height);
					$picBg.attr('src', rotates[direction].src);
					return;
				}
				...
				var img = new Image();
				img.onload = function() {
					var width = $picBg.data('height'),
						height = $picBg.data('width');
					//此处图片需跨域
					...
					realImg.onload = function() {
						...
						$picBg.data('width', width);
						$picBg.data('height', height);
						$picBg.attr('src', realImg.src);
					};
					realImg.src = src;
				};

				img.src = $picBg.attr('src');
			});
			...
			$('.save').click(function(e) {
				...
				canvas.width = $canvasBox.width()*3; //CSS中定义了画布是580
				canvas.height = $canvasBox.height()*3;
		
				var ctx = canvas.getContext('2d');
				ctx.fillStyle = '#FFF'; //绘制背景色
				ctx.fillRect(0, 0, canvas.width, canvas.height);

				poster.drawImage(ctx, rotates[direction].image, poster.intersect($canvasBox, $picBg)); // 照片
				poster.drawImage(ctx, $conBg, poster.intersect($canvasBox, $conBg)); // 下面icon和文字
				poster.drawImage(ctx, $text1, poster.intersect($canvasBox, $text1)); // 照片上文字
				poster.drawImage(ctx, $border, poster.intersect($canvasBox, $border));//时间边框
				
				// 添加时间文字
				addDayText(ctx,$day);
				addyearAndMText(ctx,$yearAndM);
				addWeekText(ctx,$week);
				
				var textareaVal = $("#textarea").val().replace(/\n/g, '_@').replace(/\r/g, '_#');
				    textareaVal = textareaVal.replace(/_@/g, '<br/>');//IE9、FF、chrome
				    textareaVal = textareaVal.replace(/\s/g, '&nbsp;');//空格处理
				var textareaVal1 = getSplitString($("#textarea").val()).split(',');	
				if (textareaVal.match('<br/>') && textareaVal1.length == 3) {
					var textareaVal1 = getSplitString($("#textarea").val()).split(',');	
					addInput1Tetxt(ctx, $input1,textareaVal1[0]);
					addInput1Tetxt(ctx, $input2,textareaVal1[1]);
					addInput3Tetxt(ctx, $input3,$input3.val());
				} else if (textareaVal.match('<br/>') && textareaVal1.length == 4) {
					var textareaVal1 = getSplitString($("#textarea").val()).split(',');	
					addInput1Tetxt(ctx, $input1,textareaVal1[0]);
					addInput1Tetxt(ctx, $input2,textareaVal1[1]);
					addInput1Tetxt(ctx, $input4,textareaVal1[2]);
					addInput3Tetxt(ctx, $input3,$input3.val());
				} else {
					addInput1Tetxt(ctx, $input1,$("#textarea").val().substring(0,14));
					addInput1Tetxt(ctx, $input2,$("#textarea").val().substring(14,28));
					addInput1Tetxt(ctx, $input4,$("#textarea").val().substring(28));
					addInput3Tetxt(ctx, $input3,$input3.val());
				}
				...
				$.ajax({					
					url:"/snowlogic/audi_calendar",
					type:"post",
					async:false,
					dataType: "json",
					data: {calendarimg:base64},
					success: function(msg) {
						if(msg.code == 200){
							window.location.href = '/calendar/share.php?subm=1&audi='+msg.data;
							return ;
						}
					},
					error: function() {
			...
			$again.click(function() {
				...
				$picBg.css('transform','translate3d(0px, 0px, 0px) scale(1)').attr('src','img/pic-bg.jpg');
				$rotate.show();
				$camera.show();
			});
		...
		function addDayText(ctx,obj) {
			var dayText = poster.intersect($canvasBox, obj);
			var pingWidth = window.innerWidth;
			var inputWidth = $('.time-i')[0].offsetWidth*1.5;
			var tnameX = dayText.frame.x+inputWidth;
			var tnameY = dayText.frame.y;
			
			var fontWidth = ctx.measureText(obj.text());
			if (pingWidth < 321) {
				ctx.font = "600 78px Arial";
				tnameY += 84;
				tnameX += 6;
			} else if (pingWidth < 350) {
				ctx.font = "600 78px Arial";
				tnameY += 69;
				tnameX += 6;
			} else if (pingWidth < 380) {
				ctx.font = "800 84px Arial";
				tnameY += 84;
				tnameX += 6;
			} else if (pingWidth < 750) {
			...
			ctx.fillStyle = "#000";
			ctx.fillText(obj.text(),tnameX, tnameY); // 写队名
		}
		
		function addyearAndMText(ctx,obj) {
			var dayText = poster.intersect($canvasBox, obj);
			var pingWidth = window.innerWidth;
			
			var inputWidth = $('.time-i')[0].offsetWidth;
			var tnameX = dayText.frame.x+inputWidth*1.5;
			var tnameY = dayText.frame.y;
		...
		function addInput1Tetxt(ctx, obj, textVal) {
			if (textVal != undefined && obj.length > 0) {
				var dayText = poster.intersect($canvasBox, obj);
				var pingWidth = window.innerWidth;
				var tnameX = dayText.frame.x;
				var tnameY = dayText.frame.y;
				var inputWidth = obj[0].offsetWidth;
				var fontWidth = ctx.measureText(textVal);
				...
				ctx.textAlign="left";
				ctx.fillStyle = "#000";
				ctx.fillText(textVal, tnameX, tnameY); // 写队名
			}
		}
		function addInput3Tetxt(ctx, obj, textVal) {
			if (obj.val() != undefined && obj.val().length > 0) {
				var dayText = poster.intersect($canvasBox, obj);
				var pingWidth = window.innerWidth;
				var inputWidth = obj[0].offsetWidth;
				var tnameX = dayText.frame.x + inputWidth*3+15;
				var tnameY = dayText.frame.y;
				var fontWidth = ctx.measureText(textVal);
...
//				touch.on(touchPad, 'rotate', function(e) {
//					
//					if (e.rotation >= 15) {
//						flag = true;
//						if (flag) {
//							$('#btnRotate').trigger("touchstart");
//							flag = false;
//						}
//						return;
//					}
	...
	function(module, exports, __webpack_require__) {
		...
		(function(global, factory) {
			...
			['width', 'height'].forEach(function(property) {
				var dimension = property.replace(/./, function(m) {
					return m[0].toUpperCase()
				});
